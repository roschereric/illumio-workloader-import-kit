...
- `examples/` — the fictional sample dataset (`sample-*-import.csv`, v1 and v2; templates only). `docs/` — the merged guide (ES/EN, branded PDF +
...
- Label model: one CSV column per label type key (`role`, `app`, `env`, `loc`, custom keys); values are plain text in
  the customer's own style, reused from `label-export` — the kit imposes no prefix/separator convention (Illumio has
  none). Workload name = "Role IP"; hostname blank unless known; interfaces `eth0:<ip>`; description
  `[<grupo> P<prio> conf:<Alta|Media|Baja>] evidence`. Examples, testdata and docs use only the fictional sample
  dataset (`examples/sample-*.csv`, generated by a script; RFC 1918 lab addressing, generic label values): never add
  customer hostnames, label values, IPs or nomenclature to the repository.
